...
def test_format_string_resolves_profile_home_with_ctx():
    ...
    ctx = _FakeCtxWithProfileHome("/opt/wayland-tester")
    out = format_string("skill_load_prompt_workflow", ctx)
    assert "/opt/wayland-tester/skills/ijfw-workflow/SKILL.md" in out, (
...
def test_format_string_resolves_profile_home_via_active_profile():
    ...
    ctx = _FakeCtxWithProfileHomeFor("tester", "/var/wayland/profiles/tester")
    out = format_string("skill_load_prompt_handoff", ctx)
    assert "/var/wayland/profiles/tester/skills/ijfw-handoff/SKILL.md" in out, (
...
def test_format_string_falls_back_to_legacy_literal():
    """No ctx + no env override -> falls back to ~/.wayland literal.
    ...
    """
    # Clear the env override for this test.
    saved = os.environ.pop("IJFW_WAYLAND_PROFILE_HOME", None)
    try:
        out = format_string("skill_load_prompt_compress", None)
        assert "{profile_home}" not in out, (
            f"unrendered placeholder leaked: {out!r}"
        )
        assert "~/.wayland/skills/ijfw-compress/SKILL.md" in out, (
            f"legacy fallback path mismatch: {out!r}"
        )
        print("  format_string falls back to ~/.wayland literal when ctx is None")
    finally:
        if saved is not None:
            os.environ["IJFW_WAYLAND_PROFILE_HOME"] = saved


def test_format_string_env_override():
    """IJFW_WAYLAND_PROFILE_HOME env var overrides the legacy fallback."""
    saved = os.environ.get("IJFW_WAYLAND_PROFILE_HOME")
    os.environ["IJFW_WAYLAND_PROFILE_HOME"] = "/tmp/wayland-env-override"
    try:
        out = format_string("skill_load_prompt_workflow", None)
        assert "/tmp/wayland-env-override/skills/ijfw-workflow/SKILL.md" in out, (
            f"env override not honoured: {out!r}"
        )
        print("  format_string honours IJFW_WAYLAND_PROFILE_HOME env override")
    finally:
        if saved is None:
            os.environ.pop("IJFW_WAYLAND_PROFILE_HOME", None)
        else:
            os.environ["IJFW_WAYLAND_PROFILE_HOME"] = saved


def test_no_hardcoded_wayland_paths_in_skill_strings():
    ...
    `~/.wayland/skills/...` literal in _strings.py the assert below catches
    ...
    for key in ("skill_load_prompt_workflow", "skill_load_prompt_handoff", "skill_load_prompt_compress"):
        ...
        assert "~/.wayland/skills" not in template, (
            f"{key} re-introduced a hardcoded ~/.wayland literal: {template!r}"
        )
...
if __name__ == "__main__":
    tests = [
        ...
        test_no_hardcoded_wayland_paths_in_skill_strings,
    ]
    failed = []
    for t in tests:
        try:
