#!/usr/bin/env python3
"""test_register.py -- verify register(ctx) wires all 6 hooks + 6 commands."""
...
def test_on_session_start_fires_mcp():
    ...
    register = build_register_fn("wayland")
    register(ctx)
    results = ctx.invoke_hook("on_session_start", session_id="s1", model="x", platform="wayland")
...
def test_pre_tool_call_pass_through():
    ...
    # Non-destructive command should return nothing (pass-through).
