...
def _should_seed(ctx, project_root):
    ...
    product -- no fourth copy to drift. ijfw_should_seed inside the script also
    refuses $HOME / its ancestors / the filesystem root.

    Fail-open (return True) when the script is unavailable so an older or
    partial install never silently loses AGENTS.md on a real project; seed-gate.sh
    ships beside lock.sh, so that path is a partial-install edge.
    """
    try:
