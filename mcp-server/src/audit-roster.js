...
export const ROSTER = [
  {
    ...
    note: 'Different training lineage; fast on review tasks. The - flag reads prompt from stdin. --skip-git-repo-check bypasses the trusted-directory gate added in codex-cli 0.118.0. --sandbox read-only blocks the model from running shell commands on the host -- empirically verified on Codex 0.118.0 (Permission denied at exec_command), and the model layer additionally refuses to read explicitly-secret files like ~/.ssh/id_rsa or ~/.codex/auth.json even when prompt-injected to do so. Layered defense, not a guarantee against future jailbreaks; full env isolation queued for 1.2.2. approval_policy="never" auto-approves without an interactive prompt. mcp_servers.ijfw-memory.enabled=false disables IJFW MCP for this session because Codex 0.118.0 in `codex exec` mode auto-cancels MCP tool calls under any non-bypass sandbox -- the cancellation noise wastes tokens and the audit does not need IJFW memory recall (the brief contains the full target inline).',
