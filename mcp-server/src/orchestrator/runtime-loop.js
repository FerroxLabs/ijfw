 ...
 *
 * STATUS (v1.5.1 cleanup C1): this module has NO production importer — S02
 * designed `reviewSubagentReport` as an MCP tool in server.js but it was never
 * wired. The genuinely-live subagent-completion path is the `subagent.post-done`
 * state-SDK verb (state-sdk.js, reachable via the `ijfw_state` MCP tool). The
 * T20-truncation + S08-worktree-guard integration that v1.5.1 W2.B+E had wired
 * into this dead module was relocated onto the live state-SDK verbs. The
 * resume-routing + termination helpers below are retained (test-covered) for a
 * future caller; nothing here executes in production today.
 */
...
// v1.5.1 cleanup C1: the W2.B (T20 truncation) + W2.E (S08 worktree-guards)
// wiring that previously lived here was wired into DEAD code — runtime-loop.js
// has zero production importers (S02 designed it as an MCP tool in server.js
// but never wired it). The orphan-module integration was RELOCATED to the
// genuinely-reachable state-SDK verbs in state-sdk.js:
//   - worktree-guards.js → `subagent.dispatch` verb (precondition guards)
//   - recovery/truncation.js (detectTruncation) → `subagent.post-done` verb
// Both are now reachable from server.js via the `ijfw_state` MCP tool. See
// .planning/1.5.1/audit/CLEANUP-SYNTHESIS-ROUND2.md item C1.
 ...
 *
 * Pure resume-routing: picks an alternate AI (`selectResumeAI`) and composes a
 * resume brief (`buildResumeBrief`). T20 truncation CLASSIFICATION + state
 * recovery were relocated to the live `subagent.post-done` state-SDK verb in
 * v1.5.1 cleanup C1 — see the import-block note above.
 *
 * @param {object} args
...
// ---------------------------------------------------------------------------
// v1.5.1 cleanup C1 — S08 worktree-guard preconditions RELOCATED
// ---------------------------------------------------------------------------
//
// The `dispatchSubagentGuarded` wrapper that previously lived here wired
// `lib/worktree-guards.js` into runtime-loop.js — which has zero production
// importers, so the guards never ran. The guard preconditions now live on the
// genuinely-reachable `subagent.dispatch` state-SDK verb (state-sdk.js
// `runWorktreeGuards`), reachable in production via the `ijfw_state` MCP tool.
