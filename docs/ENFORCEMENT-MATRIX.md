...
| **Universal** | Best-effort | `universal/ijfw-rules.md` (20-line paste-anywhere rules) | No MCP registration; no hook lifecycle. The rules file is pasted into any agent's system prompt. The LLM can write state files directly — SDK is not on the critical path. Ceiling is prompt discipline only. |
