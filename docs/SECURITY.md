...
`ijfw update`:
1. Runs `npm audit signatures @ijfw/install@<target>`. Refuses on signature failure.
2. Cross-verifies target shasum against the GitHub release asset shasum (second factor). Refuses on mismatch.
3. On success: persists `last_good_shasum = <target>` and `last_applied_version = <target>` to `state.json`.
...
Provenance attests origin, not safety. A compromised maintainer token could still sign a malicious release. This is documented limitation; the OOB confirmation step is the safety net.
