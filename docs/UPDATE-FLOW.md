...
2. **Memory prelude** (all 15 platforms) -- first-turn one-liner when behind AND `last_applied_version < last_latest_seen`
3. **Chat command** (all 14 MCP platforms) -- user types "ijfw update check" -> model invokes `ijfw_update_check` -> renders structured result + terminal confirmation instruction
